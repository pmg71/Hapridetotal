...
public class ChangePassword_driver extends AppCompatActivity implements View.OnClickListener  {
        ...
        if (connection_detector.isConnectingToInternet()) {
            ...
            try {
                ...
                Call<supervisior_Login_Response> call = api.supervisor_change_pwd(u_id.getText().toString(),
                        u_num.getText().toString(), u_pwd.getText().toString(), "driver");
                call.enqueue(new Callback<supervisior_Login_Response>() {
                    ...
                    public void onResponse(Call<supervisior_Login_Response> call,
                        ...
                        if (supervisor_loginResponse.getStatus().equalsIgnoreCase("TRUE")) {
                            Toast.makeText(getApplicationContext(), "Succesfully Changed!", Toast.LENGTH_SHORT).show();
                            Intent i1 = new Intent(getApplicationContext(), driver_login.class);
                            startActivity(i1);
                        } else {

                            Toast.makeText(getApplicationContext(),
                                    "Please enter valid Driver UserId&  Driver Mobile Number",
